...
def make_state(assistant_id=None, thread_id=None):
    ...
    return CodeExecutionState(
        user_input=UserInputState(input="테스트", user_id="u", session_id="s"),
        private=private
    )
...
def test_expire_and_renew(mock_openai, mock_create_asst):
    ...
    state = make_state()
    result1 = thread_manager_node(state, max_minutes=(0.5/60))  # 0.5초로 만료 테스트
    asst_id_1 = result1.private.assistant_id
    thread_id_1 = result1.private.thread_id
    ...
    state2 = make_state()
    result2 = thread_manager_node(state2, max_minutes=(0.5/60))
